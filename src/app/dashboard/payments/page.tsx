        ...
        <header className="min-h-[60px] sm:min-h-24 border-b border-[var(--crm-border)] flex items-center justify-between px-4 sm:px-10 bg-[var(--crm-sidebar)]/50 backdrop-blur-xl sticky top-0 z-40 py-2 sm:py-0 gap-3 sm:gap-6">
          ...
          <div className="flex items-center gap-2 sm:gap-10 shrink-0">
             <div className="flex flex-col items-end shrink-0 group relative">
                <span className="text-[6px] sm:text-[9px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] opacity-60">Tushum</span>
                <span className="text-[10px] sm:text-2xl font-black text-green-500 tracking-tighter leading-none italic">{formatMoney(totalIncome)}<span className="text-[6px] sm:text-[10px] opacity-40 uppercase ml-0.5 sm:ml-1 font-black">UZS</span></span>
                
                <div className="hidden xs:flex gap-1 sm:gap-4 mt-0.5 sm:mt-1">
                   {['CASH', 'CARD', 'TRANSFER'].map(type => {
                      ...
                      return (
                         <div key={type} className="flex flex-col items-end">
                            <span className="text-[5px] sm:text-[8px] font-black uppercase opacity-40 tracking-tighter">{labels[type]}</span>
                            <span className={`text-[6px] sm:text-xs font-black ${colors[type]} leading-tight tracking-tight`}>{formatMoney(amount)}</span>
                         </div>
                      );
                   })}
                </div>
             ...
             <button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:scale-105 transition-all text-white px-3 sm:px-10 h-10 sm:h-14 rounded-xl sm:rounded-[1.5rem] font-black text-[9px] sm:text-xs tracking-[0.1em] sm:tracking-[0.15em] flex items-center justify-center gap-1.5 sm:gap-3 shadow-2xl shadow-green-600/30 active:scale-95 uppercase whitespace-nowrap">
                ...
                <span className="hidden sm:inline">To'lov Qabul</span>
                  ...
                  <div key={p.id} className="bg-[var(--crm-card)] border border-[var(--crm-border)] rounded-[2rem] p-5 shadow-xl relative overflow-hidden group active:scale-[0.98] transition-all">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <h4 className="text-[var(--crm-text)] text-sm font-black truncate uppercase tracking-tight">{p.student?.name}</h4>
                          <p className="text-[7px] text-[var(--crm-text-muted)] font-black uppercase tracking-widest opacity-60 italic truncate max-w-[150px]">
                             {p.course?.name}
                          </p>
                        </div>
                      </div>
                      <div className="px-2 py-1 bg-[var(--crm-success-soft)] border border-[var(--crm-success-soft)] rounded-full text-green-500 text-[6px] font-black uppercase tracking-widest whitespace-nowrap shadow-sm">
                        Muvaffaqiyatli
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 pt-3 border-t border-[var(--crm-border)]/50">
                      <div>
                        <span className="text-[6px] text-[var(--crm-text-muted)] font-black uppercase tracking-widest opacity-40 block mb-0.5">Summa & Turi</span>
                        <div className="flex items-baseline gap-1">
                          <span className="text-lg font-black text-green-500 tracking-tighter leading-none">{formatMoney(p.amount)}</span>
                          <span className="text-[7px] font-black opacity-40 italic">UZS</span>
                        </div>
                        <span className="text-[7px] font-black uppercase tracking-tight text-[var(--crm-text-muted)] opacity-60 mt-0.5 block">
                          {p.paymentType === 'CASH' ? 'Naqd' : p.paymentType === 'CARD' ? 'Karta' : 'O\'tkazma'}
                        </span>
                      </div>
                      <div className="text-right">
                        <span className="text-[6px] text-[var(--crm-text-muted)] font-black uppercase tracking-widest opacity-40 block mb-0.5">Sana</span>
                        <div className="text-[10px] font-mono font-bold text-[var(--crm-text)] uppercase mb-1">
                           {new Date(p.paymentDate).toLocaleDateString("ru-RU")}
                        </div>
                        {p.user && (
                          <div className="flex items-center justify-end gap-1.5">
                             <span className="text-[7px] font-black uppercase tracking-widest text-[var(--crm-text-muted)] opacity-50 truncate max-w-[60px]">
                                {p.user.name?.split(' ')[0] || 'Admin'}
                             </span>
                             <div className="w-4 h-4 rounded-md bg-purple-500/10 border border-purple-500/10 flex items-center justify-center text-purple-600 text-[6px] font-black uppercase shadow-sm">
                                {p.user.name?.[0] || 'A'}
                             </div>
                          </div>
                        )}
                    ...
                    {p.periodFrom && (
                      <div className="mt-4 pt-3 border-t border-[var(--crm-border)]/30">
                        <span className="text-[6px] text-[var(--crm-text-muted)] font-black uppercase tracking-widest opacity-40 block mb-0.5 text-center">To'lov muddati (Period)</span>
                        <div className="text-[8px] text-[var(--crm-text-muted)] font-mono text-center opacity-60 italic">
            ...
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="w-full max-w-lg bg-[var(--crm-card)] border-t sm:border border-[var(--crm-border)] rounded-t-[2.5rem] sm:rounded-[3.5rem] p-6 sm:p-10 relative z-10 shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[92vh]">
                ...
                <header className="mb-8 relative flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                           ...
                           <p className="text-[7px] sm:text-[9px] font-black uppercase tracking-[0.2em] mt-1.5 italic opacity-60">Yangi to'lov qabul qilish</p>
                       </div>
                    </div>
                ...
                <form onSubmit={handleAddSubmit} className="space-y-4 sm:space-y-6 relative overflow-y-auto no-scrollbar pb-10">
                    <div className="space-y-1.5 px-0.5">
                        <label className="text-[8px] sm:text-[9px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.15em] ml-2 opacity-50">Talabani Tanlang</label>
                        ...
                        {selectedStudent && (
                            <div className="flex items-center justify-between px-3 mt-2">
                                <div className="flex items-center gap-2">
                                    <span className="text-[7px] font-black text-red-500/50 uppercase tracking-widest">Qarzdorlik:</span>
                                    <button 
                                        type="button"
                                    ...
                                    >
                                        <span className="text-[10px] font-black text-red-500 group-hover/pill:text-white tracking-tighter italic">{formatMoney(totalDebt(selectedStudent))}</span>
                                        <ArrowRightLeft className="w-2.5 h-2.5 text-red-500 group-hover/pill:text-white" />
                                    </button>
                                </div>
                                <span className="text-[7px] font-black text-[var(--crm-text-muted)] italic opacity-40 uppercase truncate max-w-[120px]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1.5 px-0.5">
                            <label className="text-[8px] sm:text-[9px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.15em] ml-2 opacity-50">To'lov Summasi</label>
                        ...
                        <div className="space-y-1.5 px-0.5">
                            <label className="text-[8px] sm:text-[9px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.15em] ml-2 opacity-50">Turi</label>
                            <div className="relative">
                              <select value={newPay.type} onChange={(e) => setNewPay({...newPay, type: e.target.value})} className="w-full bg-[var(--crm-bg)]/50 border border-[var(--crm-border)] rounded-2xl px-6 py-3.5 focus:border-green-500 outline-none text-[var(--crm-text)] text-[10px] font-black appearance-none cursor-pointer shadow-inner uppercase tracking-wider">
                    ...
                    <div className="grid grid-cols-2 gap-3 px-0.5">
                        <div className="space-y-1">
                            <label className="text-[7px] sm:text-[8px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.1em] ml-2 opacity-40 italic">Muddat Dan</label>
                            <input type="date" value={newPay.periodFrom} onChange={(e) => setNewPay({...newPay, periodFrom: e.target.value})} className="w-full bg-[var(--crm-bg)]/50 border border-[var(--crm-border)] rounded-xl px-4 py-2.5 focus:border-green-500 outline-none text-[var(--crm-text)] text-[10px] font-bold" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[7px] sm:text-[8px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.1em] ml-2 opacity-40 italic">Gacha</label>
                            <input type="date" value={newPay.periodTo} onChange={(e) => setNewPay({...newPay, periodTo: e.target.value})} className="w-full bg-[var(--crm-bg)]/50 border border-[var(--crm-border)] rounded-xl px-4 py-2.5 focus:border-green-500 outline-none text-[var(--crm-text)] text-[10px] font-bold" />
                        </div>
                    </div>

                    <div className="space-y-1.5 px-0.5">
                        <label className="text-[8px] sm:text-[9px] text-[var(--crm-text-muted)] font-black uppercase tracking-[0.15em] ml-2 opacity-50 italic">Izoh</label>
                        <textarea value={newPay.notes} onChange={(e) => setNewPay({...newPay, notes: e.target.value})} rows={2} className="w-full bg-[var(--crm-bg)]/50 border border-[var(--crm-border)] rounded-2xl px-6 py-3.5 focus:border-green-500 outline-none text-[var(--crm-text)] text-[10px] font-bold transition-all resize-none shadow-inner leading-relaxed" placeholder="Batafsil..." />
                    </div>

                    <div className="flex gap-3 pt-4 shrink-0">
                        <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-4 bg-[var(--crm-bg)] border border-[var(--crm-border)] text-[var(--crm-text-muted)] rounded-2xl font-black text-[9px] uppercase tracking-widest hover:bg-[var(--crm-border)] transition-all">Bekor</button>
                        <button type="submit" className="flex-[2] py-4 bg-green-600 text-white rounded-2xl font-black text-[9px] uppercase tracking-widest shadow-2xl shadow-green-600/30 hover:scale-[1.02] active:scale-95 transition-all">Qabul Qilish</button>
                    </div>
                </form>
            </motion.div>
          </div>
